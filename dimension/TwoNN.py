...
class TwoNN:
    def __init__(
        self, metric="euclidean", method="LinearFit", discard=0.2, block_analysis=True
    ):
        self.metric = metric
        self.method = method
    ...
    def fit(self, X):
        ...
        if self.block_analysis == True:
            ...
            self.BlockEstimates_ = np.column_stack(
                (BlockSize, BlockDimMeanEstimate, BlockDimStdEstimate)
            )
    ...
    def ShowBlockAnalysis(self):
        ...
        plt.plot(
            BlockEstimates[:, 0],
            np.ones(np.shape(BlockEstimates)[0]) * BlockEstimates[0, 1],
            color="blue",
        )
        plt.errorbar(
            BlockEstimates[:, 0],
